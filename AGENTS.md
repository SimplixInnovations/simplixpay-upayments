...
When **Full Automated Quality Platform** is current, `QUALITY-PLATFORM.md` is mandatory. Introduce tools progressively against named risks, commit dependency locks, keep Composer development-only until an explicit packaging migration exists, and do not convert green tooling into platform-certification claims. Keep every historical/architecture harness, `tests/harness/quality-platform-foundation-harness.php` and `tests/harness/quality-platform-static-analysis-harness.php` mandatory after Q2. The protected H12 job must always run and explicitly fail when any prerequisite quality job is not successful; dependency-skipped required checks are not acceptable gates.
