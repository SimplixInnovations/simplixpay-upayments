...
function supcheckout_provider_assert(bool $condition, string $message): void {
    if ($condition) {
        echo "PASS: {$message}\n";
        return;
    }
...
function supcheckout_provider_env(string $name): string {
    $value = getenv($name);
    return is_string($value) ? $value : '';
}

$token = supcheckout_provider_env('SUPCHECKOUT_UPAYMENTS_SANDBOX_TOKEN');
supcheckout_provider_assert(
    hash_equals('jtest123', $token),
    'only the documented public non-whitelabel sandbox token is accepted'
);

$run_id = preg_replace('/[^0-9]/', '', supcheckout_provider_env('GITHUB_RUN_ID'));
$attempt = preg_replace('/[^0-9]/', '', supcheckout_provider_env('GITHUB_RUN_ATTEMPT'));
...
$order_id = substr('supcheckout-cert-' . $run_id . '-' . $attempt, 0, 40);
$reference_id = substr('supcheckout-' . $run_id . '-' . $attempt, 0, 35);
...
supcheckout_provider_assert(
    ...
    'Charge endpoint is derived from the SUPCheckout sandbox resolver'
);
...
$payload = array(
    'products' => array(
        array(
            'name'        => 'SUPCheckout Certification',
    ...
    'order' => array(
        'id'          => $order_id,
        'reference'   => $order_id,
        'description' => 'SUPCheckout bounded sandbox initialization',
        'currency'    => 'KWD',
        'amount'      => 1.0,
    ),
    'language' => 'en',
    ...
    'returnUrl' => 'https://example.com/supcheckout-return',
    'cancelUrl' => 'https://example.com/supcheckout-cancel',
    'notificationUrl' => 'https://example.com/supcheckout-webhook',
    'plugin' => array(
        'src' => 'woocommerce',
    ),
...
supcheckout_provider_assert(is_string($json) && $json !== '', 'sandbox request JSON encodes successfully');

$ch = curl_init();
supcheckout_provider_assert($ch !== false, 'cURL handle initializes');
...
curl_setopt_array(
    ...
    array(
        ...
        CURLOPT_TIMEOUT        => 15,
        CURLOPT_USERAGENT      => 'SUPCheckout-Enterprise-Certification/0.1.0',
...
supcheckout_provider_assert($curl_errno === 0, 'sandbox Charge transport completes without cURL error');
supcheckout_provider_assert(is_string($body) && $body !== '', 'sandbox Charge returns a response body');
supcheckout_provider_assert($http_status === 201, 'sandbox Charge returns exact HTTP 201');

$decoded = json_decode($body, true);
supcheckout_provider_assert(is_array($decoded), 'sandbox Charge response is valid JSON object');
supcheckout_provider_assert(
...
supcheckout_provider_assert($normalized !== null, 'provider payment link passes the production redirect normalizer');
...
supcheckout_provider_assert($redirect_scheme === 'https', 'provider payment link uses HTTPS');
supcheckout_provider_assert(
