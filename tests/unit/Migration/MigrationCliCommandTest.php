...
final class MigrationCliCommandTest extends TestCase {
    ...
    public function test_preflight_rejects_invalid_requests_with_exact_cli_error(array $request, string $reason): void {
        ...
        try {
            ...
            self::assertSame('SUPCheckout for UPayments migration: ' . $reason, $exception->getMessage());
        }

        self::assertSame(array(array('SUPCheckout for UPayments migration: ' . $reason, true)), \WP_CLI::$errors);
    ...
    public function test_execute_requires_explicit_yes_before_request_parsing(): void {
        ...
        try {
            ...
            self::assertSame('SUPCheckout for UPayments migration: explicit_yes_required', $exception->getMessage());
        }

        self::assertSame(array(array('SUPCheckout for UPayments migration: explicit_yes_required', true)), \WP_CLI::$errors);
        self::assertSame(array(), \WP_CLI::$lines);
    }
