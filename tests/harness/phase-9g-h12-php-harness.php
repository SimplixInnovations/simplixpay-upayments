    ...
    try {
        ...
        if ($post_injected) {
            if ($post_present_before) {
                ...
                // $_POST was ABSENT before injection — restore absence.
                // #31: PHP supports unset($_POST) — it removes the superglobal
...
//             inspects the subprocess route envelope field only — that
...
//             observation — it remains useful harness routing evidence.
...
// Path classification — subprocess route envelope observation.
...
upay_assert_eq($result_success['path'] ?? null, 'store_api',
    'SP-SUCCESS-1 happy path → Store API route taken', 'harness_self_test');
...
upay_assert_eq($blocks->is_active(), true, 'BLOCKS-SAN-ACTIVE-1 enabled=yes exposes Blocks method', 'semantic_runtime');

$blocks->set_test_settings(['enabled' => 'no']);
upay_assert_eq($blocks->is_active(), false, 'BLOCKS-SAN-ACTIVE-2 enabled=no suppresses Blocks method', 'semantic_runtime');

$blocks->set_test_settings([]);
upay_assert_eq($blocks->is_active(), false, 'BLOCKS-SAN-ACTIVE-3 missing enabled flag fails closed', 'semantic_runtime');

$blocks->set_test_settings(['enabled' => true]);
upay_assert_eq($blocks->is_active(), false, 'BLOCKS-SAN-ACTIVE-4 malformed boolean enabled flag fails closed', 'semantic_runtime');

$blocks->set_test_settings(['enabled' => 'YES']);
upay_assert_eq($blocks->is_active(), false, 'BLOCKS-SAN-ACTIVE-5 noncanonical enabled token fails closed', 'semantic_runtime');
...
// — they are one authorization behavior, not 23.
//
...
//
// Group A (true parser-malformed, non-string):   6 scenarios →  48 assertions  → MALFORMED-CARD
// Group B (true parser-malformed, whitespace):   1 scenario  →   8 assertions  → MALFORMED-CARD
...
//    → 5 aliases × 8 = 40 assertions → harness_self_test (input matrix, not semantic)
...
//    → 4 aliases × 8 = 32 assertions → harness_self_test (input matrix, not semantic)
// Group C (accepted-string, downstream reject): 19 scenarios → reclassified harness_self_test
//
// Net change vs #29: MALFORMED-CARD 288 → 56 (-232 assertions). Group A', B', and C
...
$malformed_card_tokens = [
    ...
    'whitespace' => '  ',                             // string: /\s/ matches → reject_whitespace
];
...
// process_payment() to prove production plumbing is not broken — tagged
...
//   - Duplicates collapsed: WRONG-SCOPE≡PRIOR-SCOPE-SAME-GEN, WRONG-GEN≡
//     GEN-MISMATCH-CURRENT, ORPHAN-META≡ORPHAN-META-PARTIAL (each pair hits
...
//     inspection — moved to SP-HISTORY (#31 §9).
//
// 8 distinct selected-card scenarios × 7 assertions = 56 semantic_runtime
...
// This is the SAME code path production uses — not a hand-rolled mirror.
...
// #31: structured provenance spec — the harness translates this into a real
...
$sp_selected_prov_scenarios = [
    'SECRET-ABSENT' => [
        'setup' => [
            'setup_mode' => 'manual',
            'user_id' => 1,
            'secret' => null, // no secret option → read_existing_identity_context returns ABSENT
    ...
    'SECRET-MALFORMED' => [
        'setup' => [
            ...
            'secret' => [
                'version' => 1,
                'secret' => 'short', // not 64-hex → read_existing_identity_context returns INVALID
    ...
    'VALID-PROV-MEMBERSHIP-MISMATCH' => [
        'setup' => [
            ...
            'retrieve_card_token' => '99999999', // ≠ canonical token → Retrieve membership fails
        ],
        'expected_result' => 'failure',
        'expected_retrieve' => 1,
        'expected_charge' => 0,
    ...
    'VALID-PROV-MEMBERSHIP-MATCH' => [
        'setup' => [
            ...
            'retrieve_card_token' => $TOK_CANONICAL_2, // matches → Retrieve returns full membership → Charge
...
foreach ($sp_selected_prov_scenarios as $scenario_label => $scenario) {
    ...
    if (isset($scenario['setup']['provenance']) && $real_meta_key !== null) {
        ...
        // Scenario wants provenance but scope is invalid → don't write it.
        $scenario['setup']['meta'] = [];
...
// #31 §9: History/migration conditions belong in a workflow that actually
...
// harness — each scenario sets up distinct wc_get_orders() / wc_get_order()
// / order-meta fixtures and asserts the resulting externally-meaningful
// classification + reason.
//
// 8 distinct history scenarios × 7 assertions = 56 semantic_runtime additions.
...
$sp_history_scenarios = [
    ...
    'INCOMPLETE' => [
        'setup_extra' => [
            // history_total > max pages × page_size → incomplete
            'orders' => [],
            'history_total' => 250,
            'history_max_pages' => 1, // 1 × 20 = 20 < 250 → incomplete
            'orders_history_pages' => [1 => []],
        ],
...
foreach ($sp_history_scenarios as $scenario_label => $scenario) {
    ...
    $result = upay_run_store_api_child(
        ...
        '/wc/store/v1/checkout',
        'POST',
        '', // empty body — history flow doesn't consume it
        $history_setup,
        'match',
        []
    );
...
// --- I: $_POST truly absent before injection → truly absent after restoration ---
...
// #31 fix: also unset $_POST before injection (not $_POST=[] — that's PRESENCE).
upay_reset_state();
unset($_POST);
...
// --- J: state['post'] absent before injection → key remains absent afterward ---
upay_reset_state();
$state =& upay_test_state();
unset($state['post']);
...
// --- K: $_POST present-null before injection → present-null after restoration ---
...
// --- L: $_POST present-empty-array before injection → present-empty-array after restoration ---
upay_reset_state();
$_POST = [];
upay_run_process_payment(new POSTProbeGateway(), $probe_order, false, '/checkout/', 'POST', $sentinel_post);
...
// sibling (which contains no guard logic of its own — just an
...
function upay_spawn_guard_probe_child($probe_label, $harness_phrase, $kind) {
    // Residual Correction #31 — in-process probe.
    ...
    // PE-GUARD-PROBE is a recognized semantic family prefix → ledger attribution closed.
...
// PE-GUARD-PROBE → PE family (closed, no UNKNOWN_FAMILY leakage).
...
// renamed / evasive forms — case-insensitive.
// ---------------------------------------------------------------------------
...
$_semantic_ledger = [
    ...
    // CustomerTokenIdentity::get_user_meta_key() — not fake legacy keys.
...
// Section #14: Residual Correction #32 — final closure.
//
...
// is harness_self_test=641 (per directive §10). The directive ALSO
// explicitly requires new harness_self_test probes (family-order
// verification per §4, route-phrase guard probes per §7, snapshot/
// restoration verification per §5). Those additions are deliberate,
...
//   ROUTE-PHRASE-GUARD (§7): 8 phrases × 2 (reject + accept) = 16
//   FAMILY-ORDER  (§4):      3 family-rule assertions          =  3
