...
function force_refresh_user_meta($user_id) {
    $state =& upay_test_state();
    if (!empty($state['force_user_cache_refresh_failure'])) {
        return false;
    }
...
function force_refresh_order_meta($order) {
    $state =& upay_test_state();
    if (!empty($state['force_user_cache_refresh_failure'])) {
        return false;
    }
...
function wc_get_order($order_id) {
    $state =& upay_test_state();
    if (!empty($state['orders_return_null'])) {
        return null;
    }
    return isset($state['orders_fixture'][$order_id]) ? $state['orders_fixture'][$order_id] : null;
}
