...
s = p.read_text()
marker = "// Coupon/discount style persisted economics: the captured order-item total is lower\n"
if s.count(marker) != 1:
    raise SystemExit(f'expected one insertion marker, found {s.count(marker)}')

block = r'''// Real Woo coupon APIs: all three core discount types must reduce the persisted
